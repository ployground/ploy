...
def test_conn_no_fingerprint(instance, mock):
...
def test_conn(instance, mock, sshclient):
    ...
    assert conn.method_calls[1] == mock.call.connect(
        'localhost', username='root', key_filename=None, password=None, sock=None, port=22)
    assert conn.method_calls[2] == mock.call.save_host_keys(
        instance.master.known_hosts)
...
def test_conn_cached_closed(instance, mock, sshclient):
    ...
    assert second_client.method_calls[1] == mock.call.connect(
        'localhost', username='root', key_filename=None, password=None, sock=None, port=22)
    assert second_client.method_calls[2] == mock.call.save_host_keys(
        instance.master.known_hosts)
...
def test_missing_host_key_ignore(mock, tempdir, sshclient):
    ...
    key.get_fingerprint.return_value = 'foo'
    key.get_name.return_value = 'ssh-rsa'
    with mock.patch('ploy.plain.log') as LogMock:
        shkp.missing_host_key(sshclient, 'localhost', key)
    assert sshclient.mock_calls == [
        mock.call.get_host_keys(),
        mock.call.get_host_keys().add('localhost', 'ssh-rsa', key),
    ...
    assert LogMock.method_calls == [
        mock.call.warn('Fingerprint verification disabled!')]
...
def test_missing_host_key_ask_answer_no(mock, tempdir, sshclient):
    ...
    key.get_fingerprint.return_value = 'foo'
    key.get_name.return_value = 'ssh-rsa'
    with mock.patch("ploy.plain.yesno") as yesno_mock:
